...
- **Clean numeric literals**: Rely on implicit conversion for strong ID types (e.g., `race.Playernum = 1;`, `ctx.setup_game_obj(g, 1, 1);`) instead of verbose explicit casts (`player_t{1}`, `governor_t{1}`).
...
- **Document bug fixes and latent bugs in commit messages**: Every commit message body must explicitly describe any bugs, latent bugs uncovered during testing or refactoring, edge cases, or invariant violations resolved by the change. Never codify or "lock in" latent bugs with unit tests; when a latent bug is discovered, fix the underlying defect, document the fix in the commit message body, update player-facing technical documentation in `docs/` if game mechanics or formulas are affected, and assert the correct invariant in tests.
- **Variable hygiene, encapsulation, and common functions**: While modernizing and expanding coverage, actively rename cryptic or nonsensical legacy variable names to clear semantic terms, use common/shared domain functions, and strengthen entity encapsulation rather than bypassing member methods. Ensure all changes have tests proving their correctness.
- **Continuous domain documentation updates**: When discovering or clarifying how game systems and formulas work, or when fixing latent bugs that alter behavior, immediately update the relevant player-facing technical guides in `docs/` (e.g. `docs/ships.md`, `docs/combat.md`, `docs/economy.md`, `docs/navigation.md`, `docs/planets.md`) with accurate formulas and explanations (plain math, no internal C++ symbols).
