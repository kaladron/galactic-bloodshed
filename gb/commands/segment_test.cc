...
void test_segment_matrix() {
  ...
  assert(GB::commands::dispatch_command(g, GB::commands::segment_cmd,
                                        {"@@segment"}));
  std::string out = g.out.str();
  assert(out.contains("Starting segment movement..."));
  assert(out.contains("Segment completed."));
  ...
  assert(!GB::commands::dispatch_command(g, GB::commands::segment_cmd,
                                         {"@@segment", "invalid"}));
  assert(g.out.str().contains("Invalid segment number."));
