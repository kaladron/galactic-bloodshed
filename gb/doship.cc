...
void do_habitat(Ship& ship) {
  ...
  auto add =
      round_rand((double)ship.popn() * races[ship.owner() - 1].birthrate);
  ...
  switch (aimed_at.level) {
    case ScopeLevel::LEVEL_SHIP: /* ship aimed at is a legal ship now */
      ...
      if ((ship.whatorbits() == ScopeLevel::LEVEL_STAR ||
        ...
        auto range =
            std::sqrt(Distsq(ship.xpos(), ship.ypos(), s->xpos(), s->ypos()));
...
void domissile(Ship& ship, EntityManager& entity_manager) {
  ...
  if (ship.whatdest() == ScopeLevel::LEVEL_PLAN &&
    ...
    while (sh2 && !pdn) {
      if (ships[sh2]->alive() &&
          ships[sh2]->type() == ShipType::OTYPE_PLANDEF) {
    ...
    auto dist = std::sqrt(Distsq(ship.xpos(), ship.ypos(), ships[sh2]->xpos(),
                                 ships[sh2]->ypos()));
