...
export std::optional<ShipType> get_build_type(const char);
export std::optional<ScopeLevel> build_at_ship(GameObj &, Ship *, int *, int *);
...
export bool can_build_at_planet(GameObj &g, const Star &star,
                                const Planet &planet);
export bool can_build_this(const ShipType what, const Race &race, char *string);
export bool can_build_on_sector(const int what, const Race &race,
                                const Planet &planet, const Sector &sector,
                                const int x, const int y, char *string);
export int getcount(const command_t &argv, const size_t elem);
