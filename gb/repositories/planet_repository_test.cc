...
int main() {
  ...
  assert(repo.save_at_location(planet1, 1, 2));

  // Retrieve and verify
  auto retrieved1 = repo.find_by_location(1, 2);
  assert(retrieved1.has_value());
  assert(retrieved1->planet_id == 42);
  ...
  assert(repo.save_at_location(planet2, 2, 1));

  auto retrieved2 = repo.find_by_location(2, 1);
  assert(retrieved2.has_value());
  assert(retrieved2->planet_id == 43);
  ...
  assert(repo.save_at_location(planet3, 3, 0));

  auto retrieved3 = repo.find_by_location(3, 0);
  assert(retrieved3.has_value());
  assert(retrieved3->planet_id == 44);
  ...
  assert(repo.save_at_location(planet4, 4, 3));

  auto retrieved4 = repo.find_by_location(4, 3);
  assert(retrieved4.has_value());
  assert(retrieved4->planet_id == 45);
  ...
  assert(repo.save_at_location(*retrieved1, 1, 2));
  
  auto updated = repo.find_by_location(1, 2);
  assert(updated.has_value());
  assert(updated->planet_id == 42);
  ...
  // Save both to star 5
  assert(repo.save_at_location(planet5, 5, 0));
  assert(repo.save_at_location(planet6, 5, 1));

  auto p5 = repo.find_by_location(5, 0);
  auto p6 = repo.find_by_location(5, 1);
  assert(p5.has_value());
  assert(p6.has_value());
  assert(p5->planet_id == 46);
  assert(p6->planet_id == 47);
  ...
  assert(repo.save_at_location(planet7, 6, 1));

  auto retrieved7 = repo.find_by_location(6, 1);
  assert(retrieved7.has_value());
  assert(retrieved7->planet_id == 48);
