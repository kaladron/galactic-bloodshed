...
while((shipno = do_shiplist(&ship, &nextshipno)))
	...
	if(ship->type == OTYPE_VN) {
	    notify(Playernum, Governor, "You can't capture Von Neumann machines.\n");
	    free(ship);
	    continue;
	}
	...
	if (sect->owner!=Playernum) {
	    sprintf(buf,"You don't own the sector where the ship is landed [%d].\n",
		    sect->owner);
	    notify(Playernum, Governor, buf);
	    free(sect);
	    free(p);
	...
	if(argn<4)
	    what = CIV;
	else if(match(args[3], "civilians")) what = CIV;
	else if(match(args[3], "military")) what = MIL;
	else {
	...
	if(argn<3) {
	    if(what==CIV) boarders = sect->popn;
	    else if(what==MIL) boarders = sect->troops;
	} else
	...
	if((boarders > sect->popn) && what==CIV) boarders = sect->popn;
	else if((boarders > sect->troops) && what==MIL) boarders = sect->troops;
	...
	if(what==CIV) sect->popn -= boarders;
	else if(what==MIL) sect->troops -= boarders;
	...
	if(olddpopn+olddtroops) {
	    ...
	    if(astrength > 0.0)
		casualties = int_rand(0, round_rand((double)casualty_scale * (dstrength+1.0) /
						    (astrength+1.0)));

	    if(dstrength > 0.0) {
		casualties1 = int_rand(0, round_rand((double)casualty_scale * (astrength+1.0) / 
						     (dstrength+1.0)));
		casualties2 = int_rand(0, round_rand((double)casualty_scale * (astrength+1.0) / 
						     (dstrength+1.0)));
		shipdam = int_rand(0,round_rand(25. * (astrength+1.0)/(dstrength+1.0)));
	...
	sprintf(buf,"You are being attacked by%s Player #%d (%s)!!!\n",
	...
	if(!ship->alive) {
	    ...
	    sprintf(short_buf, "%s: %s [%d] DESTROYED %s\n",
		    Dispshiploc(ship), Race->name, Playernum, Ship(&s));
	}
	...
	if(casualties || casualties1 || casualties2) {
	    sprintf(buf,"Casualties: Yours: %d civ/%d mil, Theirs: %d %s\n",
		    casualties1, casualties2, casualties, what==CIV ? "civ" : "mil");
	    strcat(telegram_buf, buf);
	    sprintf(buf,"Casualties: Yours: %d %s, Theirs: %d civ/%d mil\n",
		    casualties, what==CIV ? "civ" : "mil", casualties1, casualties2);
	    notify(Playernum, Governor, buf);
	}
	warn(oldowner, oldgov, telegram_buf);
	if(ship->owner!=oldowner || !ship->alive)
	    post(short_buf, COMBAT);
	notify_star(Playernum, Governor, oldowner,
