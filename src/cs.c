	  ...
	  switch(s2->whatorbits) {
	    case LEVEL_UNIV:
	      sprintf(Dir[Playernum-1][Governor].prompt," ( [%d] /#%d/#%d )\n",
		      Sdata.AP[Playernum-1], s->destshipno, Dir[Playernum-1][Governor].shipno);
	      break;
	    case LEVEL_STAR:
	      ...
	      break;
	    case LEVEL_PLAN:
	      sprintf(Dir[Playernum-1][Governor].prompt," ( [%d] /%s/%s/#%d/#%d )\n",
		      Stars[s->storbits]->AP[Playernum-1],
		      Stars[s->storbits]->name,
		      Stars[s->storbits]->pnames[Dir[Playernum-1][Governor].pnum],
		      s->destshipno,
...
void cs(int Playernum, int Governor, int APcount)
{
  ...
  if (argn==1) {
      ...
      if ((Dir[Playernum-1][Governor].snum = Race->governor[Governor].defsystem)
	  >= Sdata.numstars)
	  Dir[Playernum-1][Governor].snum = Sdata.numstars-1;
      if ((Dir[Playernum-1][Governor].pnum = Race->governor[Governor].defplanetnum) >= Stars[Dir[Playernum-1][Governor].snum]->numplanets)
	  Dir[Playernum-1][Governor].pnum = Stars[Dir[Playernum-1][Governor].snum]->numplanets-1;
      Dir[Playernum-1][Governor].shipno = 0;
      Dir[Playernum-1][Governor].lastx[0] = Dir[Playernum-1][Governor].lasty[0] = 0.0;
	...
	if (where.level==LEVEL_STAR && where.snum==Dir[Playernum-1][Governor].snum) {
