 ...
 */
int enroll_player_race(failure_filename)
     char *failure_filename ;
{
  FILE *f, *g ;
  int n ;
  char c[128] ;
  ...
  while (critique_to_file(NULL, 1, 1)) {
    ...
    printf("Abort, enroll anyway, fix, mail rejection? [abort/enroll/fix/mail]> ") ;
    fgets(c, 16, stdin) ;
    ...
    fprintf(g, "\n") ;
    fprintf(g, "The race you submitted (%s) was not accepted, for the following reason%c:\n", race.name, (n > 1) ? 's' : '\0') ;
     ...
     char *argv[] ;
{
  ...
  printf("Enrolled %d race%c; %d failure%c saved in file %s.\n",
	 nenrolled, (nenrolled != 1)?'s':'\0', 
	 n - nenrolled, (n - nenrolled != 1)?'s':'\0', argv[2]) ;
  }
