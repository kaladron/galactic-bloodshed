...
notify(Playernum, Governor, buf);
sprintf(buf,"capacity greater than twice that the enemy can muster, including\n");
notify(Playernum, Governor, buf);
...
if (def <= 2 * attack) {
    ...
    notify(Playernum, Governor, buf);
    sprintf(buf,"You must maintain a garrison of 0.1%% the population of the\n");
    notify(Playernum, Governor, buf);
    sprintf(buf,"planet (at least %.0f); otherwise there is a 50%% chance that\n", p->popn * 0.001);
    notify(Playernum, Governor, buf);
