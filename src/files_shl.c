...
#endif
 Fileread(racedata, (char *)*r, sizeof(racetype), (rnum-1)*sizeof(racetype) );
}
...
#endif
{
  ...
  Fileread(stdata,(char *)*s, sizeof(startype), (int)(sizeof(Sdata)+star*sizeof(startype)) );
}
...
#endif
{
 ...
 fstat(shdata,&buffer);
 if (buffer.st_size / sizeof(shiptype) < shipnum) return 0;
 else {
 ...
 Fileread(shdata, (char *)*s, sizeof(shiptype),	(shipnum-1)*sizeof(shiptype) );
	return 1;
 }
}
...
#ifdef DEBUG
 if ((*c = (commodtype *)DEBUGmalloc(sizeof(commodtype), fname, lineno))==NULL)
#else
 if ((*c = (commodtype *)malloc(sizeof(commodtype)))==NULL)
#endif
		printf("getcommod:Malloc() error \n"), exit(0);

 Fileread(commoddata, (char *)*c, sizeof(commodtype), (commodnum-1)*sizeof(commodtype) );
	return 1;
 }
}
...
int getdeadcommod(void)
{
 ...
 if (buffer.st_size && (abort ==1)) {
		/* put topmost entry in fpos */
	Fileread(fd, (char *)&commodnum, sizeof(short), buffer.st_size - sizeof(short) );
		/* erase that entry, since it will now be filled */
	ftruncate(fd, (long)(buffer.st_size-sizeof(short)) );
	close_file(fd);
...
{
    Filewrite(racedata,(char *)r, sizeof(racetype), (r->Playernum-1)*sizeof(racetype));
}

void putstar(startype *s, int snum)
{
    Filewrite(stdata,(char *)s, sizeof(startype), (int)(sizeof(Sdata)+snum*sizeof(startype)) );
}
...
{
    Filewrite(sectdata, (char *)map,
	p->Maxx * p->Maxy * sizeof(sectortype), p->sectormappos);
}

void putship(shiptype *s)
{
    Filewrite(shdata,(char *)s, sizeof(shiptype), (s->number-1)*sizeof(shiptype));
}

void putcommod(commodtype *c, int commodnum)
{
    Filewrite(commoddata,(char *)c, sizeof(commodtype), (commodnum-1)*sizeof(commodtype) );
}
