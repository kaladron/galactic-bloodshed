...
void fire(int Playernum, int Governor, int APcount, int cew) /* ship vs ship */
{
  ...
  while((fromship = do_shiplist(&from, &nextshipno)))
	 ...
	 && authorized(Governor, from)) {
	  ...
	  if(cew) {
	      if(!from->cew) {
		  notify(Playernum, Governor, "That ship is not equipped to fire CEWs.\n");
		  free(from);
		  continue;
	      }
	      if(!from->mounted) {
		  notify(Playernum, Governor, "You need to have a crystal mounted to fire CEWs.\n");
		  free(from);
		  continue;
	      }
	  }
  ...
  /* save defense attack strength for retaliation */
	  ...
	  if(landed(from) && landed(to)) {
		 ...
		 (from->pnumorbits != to->pnumorbits)) {
		  notify(Playernum, Governor, "Landed ships can only attack other landed ships if they are on the same planet!\n");
		  free(from); free(to);
		  continue;
	      }
	      getplanet(&p, (int)from->storbits, (int)from->pnumorbits);
	      if(!adjacent((int)from->land_x, (int)from->land_y,
			   (int)to->land_x, (int)to->land_y, p)) {
		  notify(Playernum, Governor, "You are not adjacent to your target!\n");
		  free(from); free(to); free(p);
		  continue;
	      }
	      free(p);
	  }
	  if(cew) {
		  ...
		  notify(Playernum, Governor, "CEWs cannot originate from or targeted to ships landed on planets.\n");
		  free(from);
		  free(to);
		  continue;
	      } else {
	  ...
	  damage = shoot_ship_to_ship(from, to, strength, cew, 0,
				      long_buf, short_buf);
	  ...
	  notify_star(Playernum, Governor, (int)to->owner,
  ...
  /* defending ship retaliates */
	  ...
	  if (retal && damage && to->protect.self) {
	      ...
	      if((damage = shoot_ship_to_ship(&dummy, from, strength, 0, 1,
  ...
  /* AFVs immune to retaliation of this type */
	  if(damage && from->alive && from->type!=OTYPE_AFV) {
		  ...
		  if( ship->protect.on && (ship->protect.ship == toship)
		     ...
		     && sh != fromship && sh != toship && ship->alive 
		     && ship->active) {
		      check_retal_strength(ship, &strength);
		      if(laser_on(ship))
			  check_overload(ship, 0, &strength);
	  
		      if ((damage = shoot_ship_to_ship(ship, from, strength,
						       0, 0, long_buf, short_buf)) >= 0) {
			  if(laser_on(ship))
...
void bombard(int Playernum, int Governor, int APcount) /* ship vs planet */
{
  ...
  if(argn < 2) {
      notify(Playernum, Governor, "Syntax: 'bombard <ship> [<x,y> [<strength>]]'.\n");
      return;
  }
  ...
  while((fromship = do_shiplist(&from, &nextshipno)))
	 ...
	 authorized(Governor, from)) {
	  ...
	  if (from->whatorbits!=LEVEL_PLAN) {
	      notify(Playernum, Governor, "You must be in orbit around a planet to bombard.\n");
	      free(from);
	      continue;
  ...
  /* check to see if there are any planetary defense networks on the planet */
	  ...
	  if(!ok && !landed(from)) {
	      notify(Playernum, Governor, "Target has planetary defense networks.\nThese have to be eliminated before you can attack sectors.\n");
	      free(p);
	      free(from);
	      continue;
	  }

	  numdest = shoot_ship_to_planet(from, p, strength, x, y, 1, 0, 0,
					 long_buf, short_buf);
	  ...
	  if(numdest && from->type != OTYPE_AFV) {
		  ...
		  if (Nuked[i-1] && !p->slaved_to) {
		      ...
		      damage = shoot_planet_to_ship(alien, p, from, strength,
						    long_buf, short_buf);
		      warn(i, (int)Stars[from->storbits]->governor[i-1], long_buf);
		      notify(Playernum, Governor, long_buf);
		      if(!from->alive)
			  post(short_buf, COMBAT);
		      notify_star(Playernum, Governor, i,
				  (int)from->storbits, short_buf);
		  }
	  }
#endif
	  ...
	  if(numdest && from->alive && from->type != OTYPE_AFV) {
		  ...
		  if( ship->protect.planet && sh != fromship && ship->alive 
		     && ship->active) {
		      ...
		      if((damage = shoot_ship_to_ship(ship, from, strength, 0,
...
void defend(int Playernum, int Governor, int APcount) /* planet vs ship */
{
  ...
  if(argn < 3) {
    notify(Playernum, Governor, "Syntax: 'defend <ship> <sector> [<strength>]'.\n");
    return;
  }
  if(Governor && Stars[Dir[Playernum-1][Governor].snum]->governor[Playernum-1]!=Governor) {
      notify(Playernum, Governor, "You are not authorized to do that in this system.\n");
      return;
  }
  ...
  if(damage) {
    ...
    while(sh) {
	  ...
	  if((numdest = shoot_ship_to_planet(ship, p, strength,
					     x, y, 1, 0, 0,
					     long_buf, short_buf)) >= 0) {
...
int retal_strength(shiptype *s)
{
  ...
  if(s->guns==PRIMARY)
    avail= (s->type==STYPE_FIGHTER || s->type==OTYPE_AFV ||
	    s->type==OTYPE_BERS) ?
	s->primary : MIN(s->popn,s->primary);
  else if(s->guns==SECONDARY)
    avail= (s->type==STYPE_FIGHTER || s->type==OTYPE_AFV ||
	    s->type==OTYPE_BERS) ?
	s->secondary : MIN(s->popn,s->secondary);
  else
    avail = 0;
...
int adjacent(int fx, int fy, int tx, int ty, planettype *p)
{
	...
	else if(fx==p->Maxx-1 && tx==0) return 1;
	else if(fx==0 && tx==p->Maxx-1) return 1;
	else return 0;
    } else return 0;
}
...
void check_overload(shiptype *ship, int cew, int *strength)
{
  if ((ship->laser && ship->fire_laser) || cew) {
    if(int_rand(0,*strength) > (int)((1.0-.01*ship->damage)*ship->tech/2.0)) {
      ...
      *strength = 0;
      ...
      post(buf, COMBAT);
      notify_star((int)ship->owner, (int)ship->governor,
		  0, (int)ship->storbits, buf);
