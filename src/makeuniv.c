...
main(int argc, char *argv[])
{
  ...
  for (i=1; i<argc; i++)
    if (argv[i][0] != '-')
      ...
      switch (argv[i][1]) {
      case 'a': autoname_star = 1 ;  break ;
      case 'b': autoname_plan = 1 ;  break ;
      case 'e': planetlesschance = atoi(argv[++i]) ;  break ;
      case 'l': minplanets = atoi(argv[++i]) ;  break ;
      case 'm': maxplanets = atoi(argv[++i]) ;  break ;
      case 'p': printpostscript = 1 ;  break ;
      case 's': nstars = atoi(argv[++i]) ;  break ;
      case 'v': printplaninfo = 1 ;  break ;
      case 'w': printstarinfo = 1 ;  break ;
      default:
	printf("\n") ;
	printf("Unknown option \"%s\".\n", argv[i]) ;
      usage:
	printf("\n") ;
	printf("Usage: makeuniv [-a] [-b] [-e E] [-l MIN] [-m MAX] [-s N] [-v] [-w]\n") ;
  ...
  while ((maxplanets < minplanets) || (maxplanets > MAXPLANETS)) {
    printf("Maximum number of planets per system [%d-%d]: ", 
