...
if(mode) {
    ...
    if(argn<3) amount = max_allowed;
    else {
	sscanf(args[2], "%d", &amount);
	if(amount<=0) {
	    notify(Playernum, Governor, "You must specify a positive number of civs to arm.\n");
	    free(planet);
	    free(sect);
	    return;
	}
...
/*    enlist_cost = ENLIST_TROOP_COST * amount; */
    ...
    sprintf(buf, "%d population armed at a cost of %dd (now %d civilians, %d military)\n",
	    amount, cost, sect->popn, sect->troops);
    notify(Playernum, Governor, buf);
    sprintf(buf, "This mobilization cost %d money.\n", enlist_cost);
    notify(Playernum, Governor, buf);
} else {
    if(argn<3) amount = sect->troops;
    else {
	sscanf(args[2], "%d", &amount);
	if(amount<=0) {
	    notify(Playernum, Governor, "You must specify a positive number of civs to arm.\n");
...
/* check for defending mechs */
    mech_defend(Playernum, Governor, &people, what, planet, x, y, sect, x2, y2, sect2);
    if(!people) {
	putsector(sect, planet, x, y);
	putsector(sect2, planet, x2, y2);
	putplanet(planet,Dir[Playernum-1][Governor].snum,Dir[Playernum-1][Governor].pnum);
	free(sect); free(sect2); free(planet);
	notify(Playernum, Governor, "Attack aborted.\n");
	return;
    }

    if(sect2->owner && (sect2->owner != Playernum)) Assault = 1;
    else Assault = 0;
...
/* action point cost depends on the size of the group being moved */
    ...
    if (!enufAP(Playernum,Governor,Stars[Dir[Playernum-1][Governor].snum]->AP[Playernum-1],
		APcost)) {
	free(sect);
	free(sect2);
	putplanet(planet,Dir[Playernum-1][Governor].snum,Dir[Playernum-1][Governor].pnum);
    ...
    if (Assault) {
	ground_assaults[Playernum-1][sect2->owner-1][Dir[Playernum-1][Governor].snum] += 1;
	Race  = races[Playernum-1];
	alien = races[sect2->owner-1];
/* races find out about each other */
	alien->translate[Playernum-1] = MIN(alien->translate[Playernum-1]+5,
					    100);
	Race->translate[sect2->owner-1] = MIN(Race->translate[sect2->owner-1]+5,
					      100);
	...
	if(what==CIV)
	    sprintf(buf, "%d civ assault %d civ/%d mil\n",
		    people, sect2->popn, sect2->troops);
	else if(what==MIL)
...
/* mesomorphs absorb the bodies of their victims */
	...
	sprintf(telegram_buf, "/%s/%s: %s [%d] %c(%d,%d) assaults %s [%d] %c(%d,%d) %s\n",
		...
		Dessymbols[sect2->condition], x2, y2,
	...
	if (!(sect->popn + sect->troops + people)) {
	    ...
	    strcat(telegram_buf, buf);
/* increase modifier */
	    Race->translate[old2owner-1] = MIN(Race->translate[old2owner-1]+5,
					       100);
	}
	if (!people) {
...
/* increase modifier */
    ...
    deductAPs(Playernum, Governor, APcost, Dir[Playernum-1][Governor].snum, 0);
    x=x2; y=y2;	/* get ready for the next round */
}
notify(Playernum, Governor, "Finished.\n");
free(planet);
}
...
/* check to see if player is permited on the sector type */
	...
	notify(Playernum, Governor, "Your ships cannot walk into that sector type!\n");
	free(ship); free(p); free(sect);
	return;
    }
	...
	if(ship2->owner!=Playernum && ship2->type==OTYPE_AFV &&
	   ...
	   (ship2->land_x==x) && (ship2->land_y==y)) {
		...
		while((strength = retal_strength(ship2)) &&
		    ...
		    damage = shoot_ship_to_ship(ship2, ship,
						strength, 0, 0, long_buf, short_buf);
		    ...
		    if(strength1) {
			damage = shoot_ship_to_ship(&dummy, ship2,
						    strength1, 0, 1, long_buf, short_buf);
...
/* if the sector is occupied by non-aligned player, attack them first */
	...
	if(!isset(Race->allied,oldowner) || !isset(alien->allied,Playernum)) {
		...
		civ = (int)sect->popn; mil = (int)sect->troops;
		mech_attack_people(ship, &civ, &mil,
				   Race, alien, sect, x, y, 0,
				   long_buf, short_buf);
		notify(Playernum, Governor, long_buf);
		...
		people_attack_mech(ship, (int)sect->popn,
				   (int)sect->troops,
				   alien, Race, sect, x, y,
				   long_buf, short_buf);
		notify(Playernum, Governor, long_buf);
		...
		sect->troops = mil;
		if(!(sect->popn+sect->troops)) {
		    p->info[sect->owner-1].mob_points -=
	...
	for(i=1;i<=Num_races; i++)
		...
		notify(i, (int)Stars[Dir[Playernum-1][Governor].snum]->governor[i-1], buf);
    }
    putship(ship);
...
void mech_defend(int Playernum, int Governor, int *people, int type,
		 planettype *p, int x, int y, sectortype *s, int x2, int y2,
		 sectortype *s2)
{
    int sh;
    shiptype *ship;
	...
	if(ship->owner!=Playernum && ship->type==OTYPE_AFV && landed(ship) &&
	   retal_strength(ship) && (ship->land_x==x2) && (ship->land_y==y2)) {
		...
		while((civ+mil) && retal_strength(ship)) {
		    ...
		    mech_attack_people(ship, &civ, &mil,
				       alien, Race, s2, x2, y2, 1,
				       long_buf, short_buf);
		    notify(Playernum, Governor, long_buf);
		    warn(alien->Playernum, oldgov, long_buf);
		    if(civ+mil) {
			people_attack_mech(ship, civ, mil,
					   Race, alien, s2, x2, y2,
					   long_buf, short_buf);
			notify(Playernum, Governor, long_buf);
			warn(alien->Playernum, oldgov, long_buf);
...
void mech_attack_people(shiptype *ship, int *civ, int *mil, racetype *Race,
			...
			int ignore, char *long_msg, char *short_msg)
{
	...
	* .01 * (100.0 - (double)ship->damage)
    ...
    dstrength=
	(double)(10*oldmil*alien->fighters+oldciv)* 0.01 * alien->tech
...
void people_attack_mech(shiptype *ship, int civ, int mil, racetype *Race,
			racetype *alien, sectortype *sect, int x, int y,
			char *long_msg, char *short_msg)
{
	...
	* ((double)ship->armor+1.0)
    ...
    sprintf(short_msg, "%s: %s [%d] %s %s\n",
	    Dispshiploc(ship), Race->name, Race->Playernum,
	    ship->alive ? "attacked" : "DESTROYED", Ship(ship));
    strcpy(long_msg, short_msg);
    sprintf(buf, "\tBattle at %d,%d %s: %d civ/%d mil assault %s\n",
	    x, y, Desnames[sect->condition], civ, mil, Shipnames[ship->type]);
    strcat(long_msg, buf);
    sprintf(buf,"\tAttack: %.3f   Defense: %.3f.\n", astrength, dstrength);
    strcat(long_msg, buf);
...
void ground_attack(racetype *Race, racetype *alien, int *people, int what,
		   unsigned short *civ, unsigned short *mil,
		   unsigned int def1, unsigned int def2,
		   double alikes, double dlikes, double *astrength,
		   double *dstrength, int *casualties, int *casualties2,
		   int *casualties3)
{
	int casualty_scale;
	...
	*dstrength=(double)((*civ+*mil*10)*alien->fighters)
			* (dlikes+1.0) * ((double)def2+1.0)
			* morale_factor((double)(alien->morale-Race->morale));
	/* nuke both populations */
	casualty_scale = MIN(*people*(what==MIL ? 10 : 1)*Race->fighters,
			(*civ+*mil*10)*alien->fighters);

	*casualties = int_rand(0, round_rand((double)((casualty_scale /
						     (what==MIL ? 10 : 1))
			* *dstrength / *astrength)));
	*casualties = MIN(*people, *casualties);
	*people -= *casualties;

	*casualties2 = int_rand(0, round_rand((double)casualty_scale 
			* *astrength / *dstrength));
	*casualties2 = MIN(*civ, *casualties2);
	*civ -= *casualties2;
		/* and for troops */
	*casualties3 = int_rand(0, round_rand((double)(casualty_scale / 10)
			* *astrength / *dstrength));
