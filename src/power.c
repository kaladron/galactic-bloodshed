...
if (argn == 3 && match(args[1], "player")) {
    ...
    if (dummy_ == 0) notify(Playernum, Governor, "no blocks\n");
    else 
    ...
    if (!dummy_) notify(Playernum, Governor, "no blocks\n");
    else 
	notify(Playernum, Governor, "\n");
} else if(argn > 1) {
...
/* list the players who are in this alliance block */
    ...
    sprintf(buf,"         	       %-64.64s\n",
	    Blocks[p-1].motto);
    notify(Playernum, Governor, buf);
    sprintf(buf,"  #  Name              troops  pop  money ship  plan  res fuel dest know\n");
    notify(Playernum, Governor, buf);
...
} else {   /* list power report for all the alliance blocks (as of the last update) */
    sprintf(buf,"         ========== Alliance Blocks as of %s ==========\n",
	    Power_blocks.time);
    notify(Playernum, Governor, buf);
    sprintf(buf," #  Name             memb money popn ship  sys  res fuel dest  VPs know\n");
    notify(Playernum, Governor, buf);
    for(i=1; i<=n; i++)
	if(Blocks[i-1].VPs) {
	    ...
	    strcat(buf, temp);
	    sprintf(temp, "%5s", Estimate_i((int)(Power_blocks.ships_owned[i-1]), Race, i));
	    strcat(buf, temp);
	    sprintf(temp, "%5s", Estimate_i((int)(Power_blocks.systems_owned[i-1]), Race, i));
	    strcat(buf, temp);
	    sprintf(temp, "%5s", Estimate_i((int)(Power_blocks.resource[i-1]), Race, i));
	    strcat(buf, temp);
	    sprintf(temp, "%5s", Estimate_i((int)(Power_blocks.fuel[i-1]), Race, i));
	    strcat(buf, temp);
	    sprintf(temp, "%5s", Estimate_i((int)(Power_blocks.destruct[i-1]), Race, i));
	    strcat(buf, temp);
	    sprintf(temp, "%5s", Estimate_i((int)(Power_blocks.VPs[i-1]), Race, i));
	    strcat(buf, temp);
	    sprintf(temp, " %3d%%\n", Race->translate[i-1]);
	    strcat(buf, temp);
	    notify(Playernum, Governor, buf);
...
sprintf(buf,"         ========== Galactic Bloodshed Power Report ==========\n");
notify(Playernum, Governor, buf);

if(Race->God)
    sprintf(buf,"%s  #  Name               VP  mil  civ cash ship pl  res fuel dest morl VNs\n", argn<2?"rank" : "");
else
    sprintf(buf,"%s  #  Name               VP  mil  civ cash ship pl  res fuel dest morl know\n", argn<2?"rank" : "");
notify(Playernum, Governor, buf);
