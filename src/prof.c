...
if(argn==1) {
    ...
    sprintf(buf,"Fert:    %3d%%\t\t  Temp:\t%d\n",
	    Race->fertilize, Temp(Race->conditions[TEMP]) );
    notify(Playernum, Governor, buf);
    sprintf(buf,
	    "Rate:    %3.1f\t\t  methane  %5d%%\t      %-8.8s %c %3.0f%%\n",
	    Race->birthrate,
	    Race->conditions[METHANE],
	    Desnames[SEA], CHAR_SEA,
	    Race->likes[SEA]*100.); 
    notify(Playernum, Governor,buf);
    sprintf(buf,
    ...
    sprintf(buf,
	    ...
	    Desnames[MOUNT], CHAR_MOUNT,
	    Race->likes[MOUNT]*100.);
    notify(Playernum, Governor,buf);
    sprintf(buf,
	    "Sexes:   %1d\t\t  CO2      %5d%%\t      %-8.8s %c %3.0f%%\n",
	    Race->number_sexes,
	    Race->conditions[CO2],
	    Desnames[LAND], CHAR_LAND,
	    Race->likes[LAND]*100.);
    notify(Playernum, Governor,buf);
    sprintf(buf,
	    ...
	    Desnames[DESERT], CHAR_DESERT,
	    Race->likes[DESERT]*100.);
    notify(Playernum, Governor,buf);
    sprintf(buf,
	    "Avg Int: %3d\t\t  sulfer   %5d%%\t      %-8.8s %c %3.0f%%\n",
	    Race->IQ,
	    Race->conditions[SULFUR],
	    Desnames[FOREST], CHAR_FOREST,
