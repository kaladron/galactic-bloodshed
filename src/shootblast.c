...
int shoot_planet_to_ship(racetype *, planettype *, shiptype *, int,
			 char *, char *);
#endif
int shoot_ship_to_planet(shiptype *, planettype *, int, int, int, int, int,
			 int, char *, char *);
...
int Num_hits(double, int, int, double, int, int, int, int, int, int, int,
	     int);
...
secgundamage = 0;
hits = 0; defense = 1;
penetrate = 0; crithits = 0; critdam = 0;
if(strength <= 0) return -1;

if(!(from->alive || ignore) || !to->alive) return -1;
if(from->whatorbits==LEVEL_SHIP || from->whatorbits==LEVEL_UNIV) return -1;
...
if(laser_on(from) && from->focus) focus = 1;
else focus = 0;

if(cew) 
    hits = strength*CEW_hit((double)dist, (int)from->cew_range);
else
    hits = Num_hits((double)dist, focus, strength, from->tech,(int)from->damage,
		    fevade, tevade, fspeed, tspeed, tbody, caliber,defense);
/* CEW, destruct, lasers */
damage = 0;
if(from->mode) {
    damage = do_radiation(to, from->tech, strength,
			  hits, "radiation", damage_msg);
    ...
    if(cew) sprintf(weapon, "strength CEW");
	...
	if(from->focus)
      ...
      case LIGHT:
	sprintf(weapon, "light guns"); break;
      case MEDIUM:
	sprintf(weapon, "medium guns"); break;
      case HEAVY:
	sprintf(weapon, "heavy guns"); break;
      default:
	sprintf(weapon, "pea-shooter");
	return -1;
    }

    damage = do_damage((int)from->owner, to, (double)from->tech,
		       strength, hits, defense,
		       caliber, (double)dist, weapon, damage_msg);
    sprintf(short_msg, "%s: %s %s %s\n",
	    Dispshiploc(to), Ship(from),
	    to->alive ? "attacked" : "DESTROYED", Ship(to));
    strcpy(long_msg, short_msg);
...
hits = Num_hits(0.0, 0, strength, Race->tech, 0,
		evade, 0, speed, 0, body, MEDIUM, 1);

damage = do_damage(Race->Playernum, ship,
		   Race->tech, strength, hits, 0,
			 ...
			 char *long_msg, char *short_msg)
{
	...
	if(ship->fire_laser) caliber=LIGHT;
	else switch(ship->guns) {
	  case PRIMARY:
	...
	for (x2=0; x2<pl->Maxx; x2++) {
	    ...
	    if (d <= r) {
		    ...
		    if(s->troops &&
		       (fac > 5.0*(double)Defensedata[s->condition])) {
		...
		if(round_rand(fac) >
		   Defensedata[s->condition]*int_rand(0,10) ) {
		 ...
		 char *weapon, char *msg)
{
    ...
    fac= (2./3.14159265)*atan((double)(5*(tech+1.0)/(ship->tech+1.0)));
...
do_collateral(ship,damage,&casualties,&casualties1,
...
int Num_hits(double dist, int focus, int guns, double tech, int fdam,
...
prob = hit_odds(dist,&factor,tech,fdam,
		fev,tev,fspeed,tspeed,body,caliber,defense);
...
int hit_odds(double range, int *factor, double tech, int fdam, int fev,
	     int tev,int fspeed, int tspeed, int body, int caliber,int defense)
{
int odds;
double a, b, c;
...
double gun_range(racetype *r, shiptype *s, int mode)
{
  ...
  else return (logscale((int)(s->tech+1.0)) * SYSTEMSIZE);
}
...
{
	if(ship->laser && ship->fire_laser) return LIGHT;
	else if(ship->type==STYPE_MINE) return LIGHT;
	else if(ship->type==STYPE_MISSILE) return HEAVY;
	else if(ship->guns==PRIMARY) return ship->primtype;
	else if(ship->guns==SECONDARY) return ship->sectype;
	else return NONE;
}
		      ...
		      int *critdam, int caliber, char *critmsg)
{
    ...
    *critdam = MIN(100,*critdam);
	...
	strcat(critmsg, "CEW "); ship->cew = 0; }
    if(ship->laser && success(*critdam)) {
	strcat(critmsg, "Laser "); ship->laser = 0; }
    if(ship->cloak && success(*critdam)) {
	strcat(critmsg, "Cloak "); ship->cloak = 0; }
    if(ship->hyper_drive.has && success(*critdam)) {
	strcat(critmsg, "Hyper-drive "); ship->hyper_drive.has = 0; }
    if(ship->max_speed && success(*critdam)) {
	ship->speed = 0;
	ship->max_speed = int_rand(0,(int)ship->max_speed-1);
	sprintf(buf, "Speed=%d ", ship->max_speed); strcat(critmsg, buf);
    }
    if(ship->armor && success(*critdam)) {
	ship->armor = int_rand(0,(int)ship->armor-1);
	sprintf(buf, "Armor=%d ", ship->armor); strcat(critmsg, buf);
    }
    strcat(critmsg, "\n");
}
...
{
    return ((2./3.141592)*atan(5*(double)((attacker+1.0)/(defender+1.0))));
}
