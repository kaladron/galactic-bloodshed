...
void do_VN(shiptype *ship)
{
	...
	if (!ship->special.mind.busy) {
	    if (ship->fuel >= (float)ship->max_fuel) {
		...
		if(ship->type==OTYPE_VN) {
		    sprintf(buf,"%d resources stolen from [%d] by %c%d at %s.",
			    prod, f, Shipltrs[OTYPE_VN], ship->number,
			    prin_ship_orbits(ship));
		} else if(ship->type==OTYPE_BERS) {
		    sprintf(buf,"%d resources stolen from [%d] by %c%d at %s.",
			    prod, f, Shipltrs[OTYPE_BERS], ship->number,
			    prin_ship_orbits(ship));
		}
...
{
    /* give berserkers a mission - send to planet of offending player and bombard it */
    ship->bombard = 1;			
...
void planet_doVN(shiptype *ship, planettype *planet)
{
  ...
  if(landed(ship)) {
	...
	prod = oldres - s->resource; /* poor way for a player to mine resources */
	if(ship->type==OTYPE_VN)
	  rcv_resource(ship, prod);
	else if(ship->type==OTYPE_BERS)
	...
	rcv_fuel(ship, (float)prod);
      ...
      shipbuild = (VN_brain.Total_mad > 100 &&
		   random()&01) ? OTYPE_BERS : OTYPE_VN;
      if (ship->resource >= Shipdata[shipbuild][ABIL_COST]) {
	shiptype *s2; reg int n, numVNs;
	...
	for(j=1; j<=numVNs; j++) {
	  ...
	  s2->docked = 1;
	  s2->land_x = ((int)(ship->land_x) + int_rand(-2,2)) % planet->Maxx;
	  s2->land_x = (int)abs((float)(s2->land_x));
	  s2->land_y = ((int)(ship->land_y) + rposneg()) % planet->Maxy;
	  s2->land_y = (int)abs((float)(s2->land_y));
	  ...
	  if (shipbuild==OTYPE_BERS) {
	    ...
	    s2->mounted = 1;
	    sprintf(buf,"%s constructed %s.", Ship(ship), Ship(s2));
	    push_telegram((int)ship->owner, (int)ship->governor, buf);
